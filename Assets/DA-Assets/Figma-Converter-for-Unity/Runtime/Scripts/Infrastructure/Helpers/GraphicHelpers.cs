...
namespace DA_Assets.FCU
{
    ...
    public class GraphicHelpers : FcuBase
    {
        ...
        private bool CanDrawWithUnityImage(FObject fobject, FGraphic graphic)
        {
            ...
            if (fobject.Fills.Count(x => x.IsVisible()) != 1)
                return false;
        ...
        public bool? IsDownloadableByFills(FObject fobject, out ReasonKey reason)
        {
            ...
            if (monoBeh.IsUGUI() &&
                monoBeh.UsingUnityImage() &&
                CanDrawWithUnityImage(fobject, graphic))
            {
                reason = ReasonKey.Dl_ProceduralFillAndStrokeSupported;
                return false;
            }
