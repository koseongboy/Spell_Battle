...
namespace DA_Assets.FCU.Extensions
{
    public static class FObjectExtensionsAssembly
    {
        ...
        private static bool HasEqualRoundedSize(FObject fobject)
        {
            return fobject.Size.x.Round(FcuConfig.Rounding.IsCircle) == fobject.Size.y.Round(FcuConfig.Rounding.IsCircle);
        }

        private static bool HasCircleCornerRadius(FObject fobject)
        {
        ...
        {
            // Figma REST API returns real \n characters (U+000A), not escaped \\n.
            // Do NOT replace \n with Environment.NewLine (\r\n on Windows):
            // CharacterStyleOverrides indices map 1-to-1 to fobject.Characters UTF-16 chars.
            // Expanding \n → \r\n shifts all per-character style ranges after the first newline.
