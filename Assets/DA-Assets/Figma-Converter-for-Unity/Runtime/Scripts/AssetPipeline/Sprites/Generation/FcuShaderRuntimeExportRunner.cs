...
namespace DA_Assets.FCU
{
    public sealed class FcuShaderRuntimeExportRunner : MonoBehaviour
    {
        ...
        void RunExport()
        {
            ...
            for (int i = 0; i < manifest.Entries.Count; i++)
            {
                ...
                try
                {
                    ...
                    FigmaProject nodeProject = DAJson.FromJson<FigmaProject>(File.ReadAllText(nodeCachePath));
        ...
        static void NormalizeEntryNaming(RuntimeExportManifestEntry entry, string cacheRoot, float scale)
        {
            ...
            if (TryLoadParentFrameProject(entry.NodeId, cacheRoot, out FigmaProject parentProject))
            {
        ...
        static string TryResolveParentFrameName(string nodeId, string cacheRoot)
        {
            ...
            FigmaProject parentProject = DAJson.FromJson<FigmaProject>(File.ReadAllText(parentCachePath));
        ...
        static Rect? TryResolveCanvasBounds(RuntimeExportManifestEntry entry, FObject node, string cacheRoot, float scale)
        {
        ...
        static bool TryGetShadowStressRenderBounds(RuntimeExportManifestEntry entry, string cacheRoot, float scale, out Rect bounds)
        {
            ...
            FigmaProject renderBoundsProject = DAJson.FromJson<FigmaProject>(File.ReadAllText(nodeCachePath));
        ...
        static bool TryLoadParentFrameProject(string nodeId, string cacheRoot, out FigmaProject parentProject)
        {
            ...
            parentProject = DAJson.FromJson<FigmaProject>(File.ReadAllText(parentCachePath));
