...
{
    /// <summary>
    /// Shared helper that prepares and shows the line height adjuster window
    /// for the currently active text backend.
    /// </summary>
    public static class LineHeightAdjusterWindowHelper
    {
        ...
        private static async Task RefreshFontList(FigmaConverterUnity monoBeh, CancellationToken token)
        {
#if TextMeshPro
            if (monoBeh.UsingTextMesh())
            {
