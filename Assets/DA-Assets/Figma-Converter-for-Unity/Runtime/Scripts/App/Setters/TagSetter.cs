...
namespace DA_Assets.FCU
{
    ...
    public class TagSetter : FcuBase
    {
        ...
        private bool IsAutoSlice9(ref FObject fobject)
        {
            ...
            // Fill and stroke must be visible solid paints only; multiple solid colors are allowed.
        ...
        internal static bool EvaluateCanBeInsideSingleImage(FObject fobject, bool applyReasons, out List<string> blockers)
        {
            ...
            // Invisible TEXT nodes (visible=false) are skipped by IsEmptyRecursive and never receive
            // tags, so they would otherwise pass through via Tags.IsEmpty()=true and allow their
            // parent to be classified as singleImage (e.g. shelf → ForceImage → Downloadable).
            // In TypeScript, canBeInsideSingleImage always returns false for TEXT nodes regardless
            // of visibility. This explicit check mirrors that behavior and prevents the mismatch.
