...
namespace DA_Assets.FCU
{
    ...
    public class TransformSetter : FcuBase
    {
        ...
        public FRect GetGlobalRect(FObject fobject)
        {
            ...
            //
            // Sprite pixels are already produced by download/generation, so Unity transform
            // rotation must not be applied on top of them.
            ...
            if (isSprite)
            {
                    ...
                    !HasSelfOrAncestorRotation(fobject))
                {
                    ...
                    // Keep the RectTransform equal to Figma's logical node bounds.
                    // ImageOverflow then draws the baked renderBounds pixels outside that rect.
                    size = bSize;
                    position = bPos;
                }
                else
            ...
            List<FObject> layoutChildren = new List<FObject>();
            ...
            rect.padding = GetPadding(fobject).AdjustPadding(fobject, rect, layoutChildren);
        ...
        private static Vector4 CalculateEffectExpansionRecursive(FObject fobject)
        {
            ...
            // Clipping hides child overflow, so child extents must not expand this node.
        ...
        private static bool HasClippedRenderBounds(FObject fobject)
        {
            if (fobject.IsArcDataFilled())
                return false;
        ...
        internal async Task ApplyFigmaScaleAnchors(List<FObject> fobjects)
        {
            foreach (FObject fobject in fobjects)
            {
                if (fobject.Data?.GameObject == null)
