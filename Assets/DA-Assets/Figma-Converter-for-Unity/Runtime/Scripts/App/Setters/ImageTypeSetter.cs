...
namespace DA_Assets.FCU
{
    ...
    public class ImageTypeSetter : FcuBase
    {
        ...
        private bool CanDrawSolidFillAndStroke(FObject fobject, FGraphic graphic)
        {
            ...
            int visibleFills = fobject.Fills.Count(x => x.IsVisible());
            int visibleStrokes = fobject.Strokes.Count(x => x.IsVisible());

            return visibleFills == 1 &&
                   visibleStrokes == 1 &&
                   fobject.IndividualStrokeWeights.IsDefault();
        }

        private bool CanDrawSolidFillContainer(FObject fobject, FGraphic graphic)
        {
            if (!monoBeh.IsUGUI() || !monoBeh.UsingUnityImage())
                return false;
        ...
        private bool CanGenerateShaderSelfOnly(FObject fobject, FGraphic graphic)
        {
            if (!fobject.ContainsTag(FcuTag.Container))
                return false;
        ...
        private static bool CanGenerateSelfOnlyShape(FObject fobject, FGraphic graphic)
        {
        ...
        private static bool HasBlockedDescendantForSingleImage(FObject fobject)
        {
            if (fobject.Children.IsEmpty())
                return false;
        ...
        private bool CanGeneratePaintOrEffect(FGraphic graphic, FObject fobject)
        {
        ...
        private static bool HasVisibleEffects(FObject fobject)
        {
