...
namespace DA_Assets.FCU
{
    ...
    public sealed class HashSetter : FcuBase
    {
        ...
        private sealed class HashEngine
        {
            ...
            public int GetRenderHash(FObject root)
            {
                if (string.IsNullOrEmpty(root.Id))
                {
                    return 0;
            ...
            private int ComputeRenderHash(FObject node, bool compositeContext)
            {
                if (!node.IsVisible())
                {
                    return 0;
            ...
            private int GetChildRenderHash(FObject child, bool compositeContext)
            {
                if (string.IsNullOrEmpty(child.Id))
                {
                    return 0;
            ...
            private void AppendSelfHash(FObject fobject, bool downloadableContext, bool renderContext, ref int hash)
            {
                try
                {
            ...
            private static void AppendAutoSlice9RenderHash(FObject fobject, bool scaleInvariant, ref int hash)
            {
            ...
            private static bool ShouldHashRotation(FObject fobject)
            {
                return !fobject.IsRotationInvariantSolidCircle();
            }

            private static bool ShouldIgnoreChildrenForRenderHash(FObject fobject)
            {
                return fobject.IsGenerativeType();
            }

            private static Vector2 GetRoundedSize(FObject fobject)
            {
                return new Vector2(
                    (float)Math.Round(fobject.Size.x, FcuConfig.Rounding.BoundingSize),
                    (float)Math.Round(fobject.Size.y, FcuConfig.Rounding.BoundingSize));
            }
            ...
            private static string JoinRounded(params float[] values)
            {
                for (int i = 0; i < values.Length; i++)
                {
                    values[i] = (float)Math.Round(values[i], FcuConfig.Rounding.BoundingSize);
                }
