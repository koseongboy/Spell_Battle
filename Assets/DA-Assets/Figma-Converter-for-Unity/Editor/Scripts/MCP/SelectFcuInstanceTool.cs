...
namespace DA_Assets.FCU.MCP
{
    public class SelectFcuInstanceTool : IMcpTool
    {
        ...
        public Task<IReadOnlyList<ContentItem>> ExecuteAsync(Dictionary<string, object> args)
        {
            ...
            bool selected = FcuMcpContext.TrySelect(instanceId, out FcuMcpInstanceInfo selectedInstance);
            string json = JsonUtility.ToJson(new SelectFcuInstanceResult
            {
                selected = selected,
                instance = selectedInstance,
                message = selected
                    ? $"Selected FCU instance '{selectedInstance.name}'."
                    : $"FCU instance '{instanceId}' was not found."
            }, true);
