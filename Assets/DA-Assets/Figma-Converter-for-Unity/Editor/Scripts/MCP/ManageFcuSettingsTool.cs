...
namespace DA_Assets.FCU.MCP
{
    public class ManageFcuSettingsTool : FcuMcpToolBase
    {
        ...
        public ManageFcuSettingsTool(FigmaConverterUnity monoBeh, McpToolSO toolSO) : base(monoBeh, toolSO)
        {
        }
        ...
        private SettingsToolResponse Describe(FigmaConverterUnity fcu, Dictionary<string, object> args)
        {
        ...
        private SettingsToolResponse Get(FigmaConverterUnity fcu, Dictionary<string, object> args)
        {
            string section = NormalizeSection(GetString(args, "section", string.Empty));
        ...
        private SettingsToolResponse Set(FigmaConverterUnity fcu, Dictionary<string, object> args)
        {
        ...
        private static void RefreshOpenSettingsWindows(FigmaConverterUnity fcu)
        {
            if (fcu == null)
                return;
        ...
        private bool TryApplyMemberPatch(FigmaConverterUnity fcu, string path, object rawValue, out object appliedValue, out string error)
        {
            appliedValue = null;
        ...
        private static object ApplyValidatedSetting(FigmaConverterUnity fcu, string path, object value)
        {
        ...
        private static IEnumerable<SerializedProperty> EnumerateSerializedLeaves(FigmaConverterUnity fcu)
        {
        ...
        private static bool TryGetSerializedValue(FigmaConverterUnity fcu, string path, out SettingValue value, out string error)
        {
            value = null;
            SerializedObject so = new SerializedObject(fcu);
        ...
        private static bool TryApplySerializedPatch(FigmaConverterUnity fcu, string path, object rawValue, out object appliedValue, out string error)
        {
            appliedValue = null;
            SerializedObject so = new SerializedObject(fcu);
            SerializedProperty prop = so.FindProperty(path);
