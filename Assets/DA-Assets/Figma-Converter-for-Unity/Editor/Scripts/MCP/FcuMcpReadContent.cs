...
namespace DA_Assets.FCU.MCP
{
    internal static class FcuMcpReadContent
    {
        public static string GetInstancesJson()
        {
            ...
            {
                selected_instance_id = FcuMcpContext.GetSelectedId(),
                instances = new List<FcuMcpInstanceInfo>(FcuMcpContext.GetInstances())
            }, true);
        }

        public static string GetFrameListText(FigmaConverterUnity fcu, string emptyText, string instruction, string hierarchyPrefix, string agentDataPrefix)
        {
            SelectableFObject document = fcu.InspectorDrawer.SelectableDocument;
