Shader "D.A. Assets/Figmage/StyledRoundedRect"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            bool PathContainsUv(float2 uv, int pointCount, int maxPoints, float4 points[MAX_PATH_POINTS])
            {
                int winding = 0;

                [loop] for (int i = 0; i < (MAX_PATH_POINTS - 1); i++)
                {
                    if (i + 1 >= pointCount || i + 1 >= maxPoints)
                    {
                        break;
                    }

                    float2 aUv = points[i].xy;
                    float2 bUv = points[i + 1].xy;
                    if (points[i + 1].z > 0.5)
                    {
                        continue;
                    }
            ...
            float PathCoverage(float2 p, float2 size, int pointCount, float4 points[MAX_PATH_POINTS])
            {
                float coverage = 0.0;
                ...
                [unroll(8)] for (int yi = 0; yi < 8; yi++)
                {
                    [unroll(8)] for (int xi = 0; xi < 8; xi++)
                    {
                        ...
                        coverage += PathContainsUv(uv, pointCount, MAX_PATH_POINTS, points) ? 1.0 : 0.0;
                    }
                }
            ...
            float sdPolylinePath(float2 p, float2 size, int pointCount, int maxPoints, float4 points[MAX_PATH_POINTS])
            {
            ...
            float ShapeDistance(float2 p, float2 size, float4 radii)
            {
                ...
                {
                    return sdPolylinePath(p, size, _PathPointCount, MAX_PATH_POINTS, _PathPoints);
                }

                return sdRoundedRect(p, size, radii);
            }
            ...
            float ShapeMask(float2 p, float2 size, float4 radii)
            {
                ...
                {
                    return PathCoverage(p, size, _PathPointCount, _PathPoints);
                }
            ...
            float ContentMask(float2 p, float2 size, float4 radii, float strokeWeight, float strokeAlign, float strokeCount)
            {
                ...
                return saturate(0.5 - (distance + inset) / aa);
            }
            ...
            float FillMask(float2 p, float2 size, float4 radii, float strokeWeight, float strokeAlign, float strokeCount)
            {
                ...
                {
                    return ShapeMask(p, size, radii);
                }

                return ContentMask(p, size, radii, strokeWeight, strokeAlign, strokeCount);
            }
            ...
            float StrokeMask(float2 p, float2 size, float4 radii, float strokeWeight, float strokeAlign)
            {
                ...
                float aa = max(0.5, _Antialiasing);
                if (_StrokePathPointCount > 1)
                {
                    float strokeDistance = sdPolylinePath(p, size, _StrokePathPointCount, MAX_PATH_POINTS, _StrokePathPoints);
                    if (abs(strokeDistance) < 1.0)
                    {
                        return PathCoverage(p, size, _StrokePathPointCount, _StrokePathPoints);
                    }

                    return saturate(0.5 - strokeDistance / aa);
                }

                float distance = ShapeDistance(p, size, radii);
                float outer = (_PathPointCount > 1 && abs(distance) < 1.0)
                    ? PathCoverage(p, size, _PathPointCount, _PathPoints)
                ...
                if (strokeAlign < 1.5)
                {
                    ...
                    return saturate(outer2 - inner2);
                }

                float outer3 = saturate(0.5 - (distance - strokeWeight) / aa);
                return saturate(outer3 - outer);
            }
            ...
            float4 Composite(float4 dst, float4 src, int mode)
            {
                ...
                if (mode == 2 && dstAlpha > 1e-5 && srcAlpha > 1e-5)
                {
                    ...
                    return float4(SrgbToLinear(resSrgb), resAlpha);
                }

                if (mode != 0 && dstAlpha > 1e-5 && srcAlpha > 1e-5)
                {
                    float3 dstSrgb = LinearToSrgb(dst.rgb);
                    float3 srcSrgb = LinearToSrgb(src.rgb);
                    float3 blendedSrgb = BlendRgbSrgb(dstSrgb, srcSrgb, mode);
                    ...
                    float3 outSrgb = outAlphaSrgb > 1e-5 ? outPremulSrgb / outAlphaSrgb : 0.0;
                    return float4(SrgbToLinear(outSrgb), outAlphaSrgb);
                }
                ...
                return float4(SrgbToLinear(outSrgb), outAlpha);
            }
            ...
            float4 EvaluateScene(float2 screenUv)
            {
                ...
                float4 result = _UseInputBodyTex > 0.5 ? tex2D(_InputBodyTex, screenUv) : float4(0, 0, 0, 0);
