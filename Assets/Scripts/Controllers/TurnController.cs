...
namespace Controllers.TurnControllers {
    public class TurnController : NetworkBehaviour {
        ...
        private void AdvancePhaseLogic() {
            switch (model.CurrentPhase.Value) {
                case GamePhase.Wait:
                    ...
                    break;
                case GamePhase.Draw:
                    model.CurrentPhase.Value = GamePhase.Select;
                    break;
                case GamePhase.Select:
                    model.CurrentPhase.Value = GamePhase.Incantation;
                    break;
                case GamePhase.Incantation:
        ...
        // [클라이언트 -> 서버] 서류철 제출
        [Rpc(SendTo.Server, InvokePermission = RpcInvokePermission.Everyone)]
        private void SubmitSpellServerRpc(int[] cardIds, string evalJson, int declaredCost,
